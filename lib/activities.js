...
Meteor.methods({
  createActivity: function(activity, tweet, facebook, blob) {
...
var fbpostActivity = function(activity, blob) {
  if (Meteor.isClient) {
    return;
  }
  ...
  var response = null;
  if (activity.image) {
    var route = 'http://requestb.in/rkfoh6rk';
    parts = {
      message: text,
      access_token: "nopenopenope",
      attachment: {
