...
if (Meteor.isServer) {
  var facebookOauth = function(options) {
    var config = Meteor.settings.facebook;
    var userConfig = Meteor.user().services.facebook;
...
var fbpostActivity = function(activity) {
  ...
  var userId = Meteor.user().services.facebook;
  ...
  if (activity.image) {
    ...
    var response = HTTP.post(
      'https://graph.facebook.com/v2.5/' + userId + '/photos', {
        params: {
          caption: (activity.text + ' #cde2015'),
          source: image,
        },
        npmRequestOptions: { oauth: facebookOauth() }
      }
    );
  } else {
    var response = HTTP.post(
      'https://graph.facebook.com/v2.5/' + userId + '/feed', {
        params: {
          message: (activity.text + ' #cde2015'),
        },
        npmRequestOptions: { oauth: facebookOauth() }
      }
    );
